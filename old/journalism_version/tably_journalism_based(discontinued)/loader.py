...
def from_file(filepath, delimiter=None, xlsheetname="Sheet1"):
    ...
    # finally determine fieldtypes
    fieldmapping,rows = detect_fieldtypes(fields, rows)
...
def detect_fieldtypes(fields, rows):
    fieldmapping = []
    for fieldname,values in zip(fields, rows):
        # test for numbers
        ...
        # test for boolean
            ...
            fieldtype = "boolean"
        # test for dates
        elif all( [isinstance(val, datetime.datetime) for val in values] ):
            fieldtype = "dates"
        # make text as last resort
        else:
            fieldtype = "text"
        fieldmapping.append((fieldname,fieldtype))
    return fieldmapping, rows
