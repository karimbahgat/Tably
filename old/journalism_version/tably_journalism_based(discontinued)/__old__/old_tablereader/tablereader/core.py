...
class Table:
    ...
    def _LoadFromFile(self, tablepath=None, data=None, delimiter="not specified", xlsheetname=None, displaydetails=False):
        ...
        if self.filetype in ("txt","csv"):
            ...
            self.fields = self.readerobj[0]
        elif self.filetype == "excel":
            workbook = xlrd.open_workbook(tablepath)
            self.excelobj = workbook.sheet_by_name(xlsheetname)
            self.readerobj = [ [txt(self.excelobj.cell_value(row, column)) for column in range(self.excelobj.ncols)] for row in range(self.excelobj.nrows)]
...
##                spss = savReaderWriter.SavReader(tablepath)
##                spssvalueLabels = spss.valueLabels
##                spssvarNames = spss.varNames
##                varnames = [eachvar for eachvar in spssvarNames]   #choose varlabels for fieldlist to provide more context
##                readerlist = [varnames]
##                for r, eachrow in enumerate(spss):
##                    labeledrow = []
...
##                    readerlist.append(labeledrow)
##                locreader = readerlist
##                return locreader
            ...
            self.fields = self.readerobj[0]
        #now that fieldnames have been saved, disable reading top fields as part of table
        if self.filetype != "dbf" and self.filetype != "shp":
            self.readerobj = self.readerobj[1:]
        #detemine rowcount
        Report("the table has "+txt(self.len)+" rows")
        #other stuff
        self.joinconditions = dict()
        self.tempoutput = list()
        #add unique rowid if file is shapefile
        if self.filetype == "shp":
            self.AddUniqueID("SHAPEID")
        self.invisiblefields = ["SHAPEID"]
    
    #define some functions to be used by user
    def FindFieldNameIndex(self, fieldname):
        """
        Internal use only.
        """
        return self.fields.index(fieldname)
    def FetchEntireRow(self, row):
        """
    ...
    def ReadRow(self, row, field):
        ...
        if isinstance(field, int) == True:
            ...
            column = self.FindFieldNameIndex(field)
        ...
        #if isinstance(readvalue, str):
        #    readvalue = txt(readvalue)
        return readvalue
    @property
    def len(self):
    ...
    def SortTable(self, sortfields, direction="down"):
        """
        ...
        if not isinstance(sortfields, list):
            Report("error: the sortfields argument in the SortTable method must be a list")
            return
        ...
        #run sort
        sortfieldindexes = [ self.FindFieldNameIndex(eachsort) for eachsort in sortfields ]
    ...
    def Save(self, savepath):
        ...
        if self.filetype == "shp":
            ...
            self.SortTable(sortfields=["SHAPEID"])
        ...
        # finished
        Report(savepath)
    def AddJoinCondition(self, mainfield, joinfield, matchpercent=100):
        """
    ...
    def CreateJoin(self, jointable, mergematches=False, fieldmapping=(), keepall=True, displaydetails=False):
            ...
            for index, row in enumerate(table):
                ...
                for field in fieldfields:
                    ...
                    if value not in blankvalues:
                        flatvalues_nodupl.add(CleanName(value))
                        flatvalues.update( {flatindex:CleanName(value)} )
        ...
        for joinconditionID in self.joinconditions:
            ...
            # collect values
            joinlookup = flattentable(jointable.ReturnAsList(), joinfield)
            # write values
            ...
            for rowindex in range(self.len):
                ...
                while joinconditionindex < len(self.joinconditions):
                    ...
                    while eachmainfieldindex < len(mainfield):
                        ...
                        eachmainfieldvalue = CleanName(self.ReadRow(rowindex, eachmainfield))
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def LoadFile(filepath, xlsheetname=None):
    """
...
##    #join
##    table1.AddJoinCondition(mainfield="Countries",joinfield="Countries2",matchpercent=90)
