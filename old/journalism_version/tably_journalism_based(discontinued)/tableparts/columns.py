...
from ..dependencies import dateutil; from dateutil import parser; parse = parser.parse
from ..dependencies import six

from .exceptions import ColumnDoesNotExistError, NullComputationError, CastError
...
class ColumnMapping(Mapping):
    ...
    def __unicode__(self):
        fieldstrings = ("%s (type: %s)" %(six.text_type(fieldname),six.text_type(fieldtype))
                               for fieldname,fieldtype in itertools.izip(self._table.fieldnames, self._table.fieldtypes) )
        return "\n".join([ "Variables:",
                           "\t"+"\n\t".join(fieldstrings) ])
...
class Column(Sequence):
    ...
    def __unicode__(self):       
        ...
        values = list( sorted ( set([six.text_type(cell) for cell in data]) ) )

        if len(values) > 40:
            values = values[:40]
            values.append('...')

        return "\n".join(["Column instance: %s" %self._table.fieldnames[self._index],
                          "Data type: %s" %self._table.fieldtypes[self._index],
                          "Unique values:",
                          "\t"+"\n\t".join(values) ])
