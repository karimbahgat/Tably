...
from .exceptions import ColumnDoesNotExistError, RowDoesNotExistError
...
class RowSequence(Sequence):
    ...
    def __unicode__(self):
        topfields = list(self._table.fieldnames[:5])

        if len(self._table.fieldnames) > 5:
               topfields.append("...")
        
        data = [six.text_type(d) for d in self[:20] ]

        if len(self._table) > 20:
            data.append("...")

        return "\n\t".join(["Rows:",
                            "Fields ( %s )" %", ".join(topfields),
                            "\n\t".join( data ) ])
